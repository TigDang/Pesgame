  ...
  <div class="wrapper">
    ...
    <div class="main_block container">
      <div class="main_block_body">
        <div class="task_text"><span>Задание:</span> Перед вами представлены карточки. Выберите все те, на которых изображены достопремечательности европейских стран</div>
        <div class="cards">
        ...
        <div v-bind:class="{'end_block': !isVisibleAgain && isVisibleResult, 'not_visible': !isVisibleResult || isVisibleAgain}">
          <div class="again_text">Поздравляем с успешным окончанием игры!</div>
          <router-link :to="{name: 'main_menu', params:{name:username, allScore:score}}" class="next_button end_button" >Результаты игры</router-link>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods:{
    ...
    getTheme(){
      ...
      const buttons_background = document.querySelectorAll('.check_button, .next_button, .end_button');
    ...
    results () {
      ...
      if(this.scoreLevel > parseInt(sessionStorage.getItem('scoreLevel6'))) {
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) - parseInt(sessionStorage.getItem('scoreLevel6')))
        sessionStorage.setItem('scoreLevel6', this.scoreLevel)
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) + parseInt(sessionStorage.getItem('scoreLevel6')))
      }
      this.score = parseInt(sessionStorage.getItem('allScore'))
    },
