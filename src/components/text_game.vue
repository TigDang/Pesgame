        ...
        <div  v-bind:class="{'down_block': isVisibleResult, 'not_visible': !isVisibleResult}">
          ...
          <div class="button_block">
            <div v-if="!isVisibleAgain">
              <router-link :to="{name: 'card_game', params:{name:username, allScore:score}}" class="next_button" >Следующий уровень</router-link>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      text: 'Наступила золотая осень ура. Небо стало более тусклым цвет, в воздухе появился свежий запах. Хотя он еще теплый, но уже не такой, как зимним летом. Все говорит о том, что природа людей, спустя пару месяцев, погрузится в безмятежный зимний сон медведя. Ночи становятся длиннее длины, а дни короче говоря. Все чаще и чаще в небе видны стаи перелетных птиц, которые направляются в теплые края. Все, что происходит, навевает некоторую соленую грусть, ведь тихое "умирание" природы почему-то всегда напоминает о том, что человеческая жизнь тоже не конечна.',
      words: [], //массив слов
      selected: [], //массив выбранных слов
      allWords: [], //массив объекта кнопки со словом
      incorrect: [3, 8, 23, 31, 40, 44, 48, 69, 84], //массив с индексами лишних слов
  ...
  methods: {
    ...
    results() {
      ...
      if(this.scoreLevel > parseInt(sessionStorage.getItem('scoreLevel1'))) {
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) - parseInt(sessionStorage.getItem('scoreLevel1')))
        sessionStorage.setItem('scoreLevel1', this.scoreLevel)
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) + parseInt(sessionStorage.getItem('scoreLevel1')))
      }
      this.score = parseInt(sessionStorage.getItem('allScore'))
    },
