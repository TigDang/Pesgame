    ...
    <div class="main_block container">
      <div class="main_block_body">
        <div class="task_text"><span>Задание:</span> Перед вами представлен текст, выберите из него все слова, которые
          являются лишними, то есть, не подходят в предложениях по смыслу или которые написаны с ошибками.
        </div>
        ...
        <div v-bind:class="{'down_block': isVisibleResult, 'not_visible': !isVisibleResult}">
          ...
          <div class="button_block">
            <div v-if="!isVisibleAgain">
              <router-link :to="{name: 'card_game_3', params:{name:username, allScore:score}}" class="next_button" >Следующий уровень</router-link>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      incorrect: [3, 8, 13, 22, 25, 30, 33, 39, 58, 62, 67, 91, 99], //массив с индексами лишних слов
  ...
  methods: {
    ...
    results() {
      ...
      if(this.scoreLevel > parseInt(sessionStorage.getItem('scoreLevel5'))) {
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) - parseInt(sessionStorage.getItem('scoreLevel5')))
        sessionStorage.setItem('scoreLevel5', this.scoreLevel)
        sessionStorage.setItem('allScore', parseInt(sessionStorage.getItem('allScore')) + parseInt(sessionStorage.getItem('scoreLevel5')))
      }
      this.score = parseInt(sessionStorage.getItem('allScore'))
    },
